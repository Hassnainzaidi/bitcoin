...
The bpftrace examples contain a relative path to the `viraliumd` binary. By
default, the scripts should be run from the repository-root and assume a
self-compiled `viraliumd` binary. The paths in the examples can be changed, for
...
```
$ python3 contrib/tracing/p2p_monitor.py $(pidof viraliumd)
```
...
```
$ python3 contrib/tracing/log_raw_p2p_msgs.py $(pidof viraliumd)
```
...
```
$ ./build/bin/viraliumd -signet -reindex
```
...
```bash
$ python3 contrib/tracing/log_utxocache_flush.py $(pidof viraliumd)
```
...
```console
$ python3 contrib/tracing/mempool_monitor.py $(pidof viraliumd)
```
