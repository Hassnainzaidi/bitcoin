...
`/Applications/Bitcoin-Qt` (on Mac) or `viraliumd`/`bitcoin-qt` (on
Linux).
...
`contrib/init/viraliumd.service`, it has been changed to use
`/var/lib/viraliumd` as the data directory instead of
`~bitcoin/.bitcoin`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/viraliumd` will
exist has enough space (check using `df -h /var/lib/viraliumd`), and
...
The systemd init file (`contrib/init/viraliumd.service`) has been changed
to use `/var/lib/viraliumd` as the data directory instead of
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart viraliumd with the `-deprecatedrpc=generate`
...
- #14162 Also log and print messages or questions like viraliumd (MarcoFalke)
