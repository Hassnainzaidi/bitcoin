...
xattr -d com.apple.quarantine bitcoin-cli bitcoin-qt bitcoin-tx bitcoin-util bitcoin-wallet viraliumd test_bitcoin
codesign -s - bitcoin-cli bitcoin-qt bitcoin-tx bitcoin-util bitcoin-wallet viraliumd test_bitcoin
```
...
- When the `-port` configuration option is used, the default onion listening port will now
  ...
  127.0.0.1:5556`, or configure viraliumd with `-bind=127.0.0.1:8334=onion` to get the previous behavior.
  (#31223)
