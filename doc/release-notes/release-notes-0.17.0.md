...
be used in V0.17 by starting viraliumd with the '-deprecatedrpc=accounts'
...
- New `fees` field introduced in `getrawmempool`, `getmempoolancestors`, `getmempooldescendants` and
   ...
   and `descendant` denominated in VIR. This new field deprecates previous fee fields, such as
...
- `signrawtransaction` is deprecated and will be fully removed in v0.18. To use
  `signrawtransaction` in v0.17, restart viraliumd with
...
- When running viraliumd with `-debug` but without `-daemon`, logging to stdout
...
meaning the `-txindex` flag can be toggled without a full reindex. If viraliumd
...
- #12240 `6f8b345` Introduced a new `fees` structure that aggregates all sub-field fee types denominated in VIR (mryandao)
...
- #12102 `7fb8fb4` Apply hardening measures in viraliumd systemd service file (Flowdalic)
- #12652 `55f490a` bitcoin-cli: Provide a better error message when viraliumd is not running (practicalswift)
...
- #12769 `35eb9d6` Add systemd service to viraliumd in debian package (ghost)
...
- #12877 `2b54155` Use viraliumd in Tor documentation (knoxcard)
