...
* Manage regtest viraliumd subprocesses.
* Access RPC interfaces of the underlying viraliumd instances.
...
* Python3
* `viraliumd` built in the same repository as the `TestShell`.
...
The following `TestShell` methods manage the lifetime of the underlying viraliumd
...
temporary folder. If you need more viraliumd nodes than set by default (1),
...
viraliumd subprocesses (nodes) and logging utilities running until the user
explicitly shuts down the `TestShell` object.

During the time between the `setup` and `shutdown` calls, all `viraliumd` node
...
useful in constructing objects which can be passed to the viraliumd nodes managed
by a running `TestShell` object.

## 5. Shutting the `TestShell` down

Shutting down the `TestShell` will safely tear down all running viraliumd
...
The following utility consolidates logs from the viraliumd nodes and the
...
|---|---|---|
| `bind_to_localhost_only` | `True` | Binds viraliumd P2P services to `127.0.0.1` if set to `True`.|
| `cachedir` | `"/path/to/bitcoin/build/test/cache"` | Sets the viraliumd datadir directory. |
| `chain`  | `"regtest"` | Sets the chain-type for the underlying test viraliumd processes. |
| `configfile` | `"/path/to/bitcoin/build/test/config.ini"` | Sets the location of the test framework config file. |
| `coveragedir` | `None` | Records viraliumd RPC test coverage into this directory if set. |
| `loglevel` | `INFO` | Logs events at this level and higher. Can be set to `DEBUG`, `INFO`, `WARNING`, `ERROR` or `CRITICAL`. |
| `nocleanup` | `False` | Cleans up temporary test directory if set to `True` during `shutdown`. |
| `noshutdown` | `False` | Does not stop viraliumd instances after `shutdown` if set to `True`. |
| `num_nodes` | `1` | Sets the number of initialized viraliumd processes. |
| `perf` | False | Profiles running nodes with `perf` for the duration of the test if set to `True`. |
| `rpc_timeout` | `60` | Sets the RPC server timeout for the underlying viraliumd processes. |
...
| `usecli` | `False` | Uses the bitcoin-cli interface for all viraliumd commands instead of directly calling the RPC server. Requires `supports_cli`. |
