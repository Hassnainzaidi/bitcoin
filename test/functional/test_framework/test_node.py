...
"""Class for viraliumd node under test"""
...
class TestNode():
    """A class for representing a viraliumd node under test.
    ...
    def __init__(self, i, datadir_path, *, chain, rpchost, timewait, timeout_factor, binaries, coverage_dir, cwd, extra_conf=None, extra_args=None, use_cli=False, start_perf=False, use_valgrind=False, version=None, v2transport=False, uses_wallet=False):
        ...
        # This means that starting a viraliumd using the temp dir to debug a failed test won't
        # spam debug.log.
    ...
    def __del__(self):
        # Ensure that we don't leave any viraliumd processes lying around after
        # the test ends
        if self.process:
    ...
    def start(self, extra_args=None, *, cwd=None, stdout=None, stderr=None, env=None, **kwargs):
        ...
        # If listening and no -bind is given, then viraliumd would bind P2P ports on
        ...
        # Add a new stdout and stderr file each time viraliumd is started
        if stderr is None:
        ...
        # unclean shutdown), it will get overwritten anyway by viraliumd, and
        ...
        self.running = True
        self.log.debug("viraliumd started, waiting for RPC to come up")

        if self.start_perf:
            self._start_perf()

    def wait_for_rpc_connection(self, *, wait_for_import=True):
        """Sets up an RPC connection to the viraliumd process. Returns False if unable to connect."""
        ...
        for _ in range(poll_per_s * self.rpc_timeout):
            if self.process.poll() is not None:
                ...
                raise FailedToStartError(self._node_msg(
                    f'viraliumd exited with status {self.process.returncode} during initialization. {str_error}'))
            try:
                rpc = get_rpc_proxy(
                ...
                # Suppress if cookie file isn't generated yet and no rpcuser or rpcpassword; viraliumd may be starting.
                if "No RPC credentials" not in str(e):
                    raise
                suppressed_errors["missing_credentials"] += 1
                latest_error = repr(e)
            time.sleep(1.0 / poll_per_s)
        self._raise_assertion_error(f"Unable to connect to viraliumd after {self.rpc_timeout}s (ignored errors: {str(dict(suppressed_errors))}, latest error: {latest_error})")
    ...
    def wait_for_cookie_credentials(self):
        ...
        for _ in range(poll_per_s * self.rpc_timeout):
            try:
                ...
                return
            except ValueError:  # cookie file not found and no rpcuser or rpcpassword; viraliumd is still starting
    ...
    def wait_until_stopped(self, *, timeout=viraliumd_PROC_WAIT_TIMEOUT, expect_error=False, **kwargs):
    ...
    def _start_perf(self, profile_name=None):
        ...
        if not test_success('readelf -S {} | grep .debug_str'.format(shlex.quote(self.binary))):
            self.log.warning(
                "perf output won't be very useful without debug symbols compiled into viraliumd")
    ...
    def assert_start_raises_init_error(self, extra_args=None, expected_msg=None, match=ErrorMatch.FULL_TEXT, *args, **kwargs):
        ...
        extra_args: extra arguments to pass through to viraliumd
        expected_msg: regex that stderr should match when viraliumd fails

        Will throw if viraliumd starts without an error.
        Will throw if an expected_msg is provided and it does not match viraliumd's stdout."""
        ...
        with tempfile.NamedTemporaryFile(dir=self.stderr_dir, delete=False) as log_stderr, \
            ...
            try:
                ...
                self.log.debug(self._node_msg(f'viraliumd exited with status {ret} during initialization'))
                ...
                self.running = False
                self.process = None
                assert_msg = f'viraliumd should have exited within {self.rpc_timeout}s '
