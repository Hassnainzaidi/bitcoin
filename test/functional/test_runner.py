...
def main():
    ...
    enable_viraliumd = config["components"].getboolean("ENABLE_viraliumd")

    if not enable_viraliumd:
...
def run_tests(*, test_list, build_dir, tmpdir, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, use_term_control, results_filepath=None):
    ...
    # Warn if viraliumd is already running
    try:
        # pgrep exits with code zero when one or more matching processes found
        if subprocess.run(["pgrep", "-x", "viraliumd"], stdout=subprocess.DEVNULL).returncode == 0:
            print("%sWARNING!%s There is already a viraliumd process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    except OSError:
        # pgrep not supported
        pass
